...
const SuggestChangeModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Compose a descriptive natural-language message from the structured form
      const actionLabel = actionOptions.find(a => a.id === selectedAction)?.label || selectedAction;
      const contextParts: string[] = [];
      if (dayNumber && dayTitle) contextParts.push(`Day ${dayNumber} (${dayTitle})`);
      if (eventTitle) contextParts.push(`regarding "${eventTitle}"`);
      const context = contextParts.length > 0 ? ` — Context: ${contextParts.join(', ')}` : '';
      const prefPart = selectedPreferences.length > 0
        ? ` Preferences: ${selectedPreferences.join(', ')}.`
        : '';

      const message = `[${actionLabel}]${context}: ${details.trim()}.${prefPart}`;

      await apiClient.submitFeedbackMessage(message);

      // Still call the parent callback if provided (for optimistic UI updates)
      if (onSubmit) {
      ...
      console.error('[SuggestChangeModal] Failed to submit feedback:', err);
      setIsSubmitting(false);
      alert(`Could not send suggestion: ${err?.message || 'Please try again.'}`);
    }
  };
      ...
      <div className="bg-[#FDFDFF] rounded-2xl max-w-2xl w-full my-8 animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-3">
              {actionOptions.map((action) => (
                <button
                  ...
                  className={`p-4 rounded-xl border-2 transition-all text-left ${selectedAction === action.id
            ...
            <div className="flex flex-wrap gap-2">
              {preferenceChips.map((pref) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedPreferences.includes(pref)
        ...
        <div className="p-6 border-t border-[#212121]/10 flex gap-3">
          ...
          <button
            ...
            className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all ${!selectedAction || !details.trim()
