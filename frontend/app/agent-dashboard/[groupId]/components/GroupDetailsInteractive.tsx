                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                            {allGroups.map((group) => (
                                <Link
                                    ...
                                    className={`p-3 rounded-xl border-2 transition-all hover:shadow-md ${group.id === selectedGroup.id
                                            ? 'bg-slate-900 text-white border-slate-900'
                                            : 'bg-white text-slate-700 border-slate-200 hover:border-slate-300'
                                        }`}
                                >
                                    <div className="flex items-center justify-between mb-1">
                                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${group.id === selectedGroup.id
                                                ? 'bg-slate-700 text-slate-200'
                                                : 'bg-slate-100 text-slate-500'
                                            }`}>
                                            {group.id}
                                        </span>
                                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${group.status === 'booked'
                                    ...
                                    <p className={`text-[10px] mt-1 truncate ${group.id === selectedGroup.id ? 'text-slate-300' : 'text-slate-500'
                                        }`}>
                                        {group.destination}
                                    </p>
                                </Link>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className={`px-3 py-1 text-xs font-bold rounded-full uppercase tracking-wide ${groupDetails.status === 'Active'
                                                                    ...
                                                                    <div className="flex items-start justify-between">
                                                                        <div className="flex-1">
                                                                            <div className="flex items-center gap-2 mb-1">
                                                                                ...
                                                                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${member.role === 'Head'
