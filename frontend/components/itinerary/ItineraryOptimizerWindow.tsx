    ...
    useEffect(() => {
        async function loadTrips() {
            try {
                ...
                // Check for locally mocked trips generated by the New Itinerary Page
                try {
                    const localTripsStr = localStorage.getItem('agent_mock_trips');
                    if (localTripsStr) {
                        const localTrips = JSON.parse(localTripsStr) as Trip[];
                        if (localTrips.length > 0) {
                            setTrips(prev => [...localTrips, ...prev]);
                        }
                    }
                } catch (e) {
                ...
                <div className="px-6 md:px-8 pt-4 md:pt-6 pb-4 max-w-7xl mx-auto">
                    <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3 shrink-0 relative">
                            ...
                            <button
                                onClick={() => router.push('/agent-dashboard/itinerary-management/new')}
