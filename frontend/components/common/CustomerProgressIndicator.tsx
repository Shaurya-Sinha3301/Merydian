...
const CustomerProgressIndicator = ({ 
  ...
  return (
    <div className={`neu-flat border-b border-border/50 ${className}`}>
              ...
              <div key={step.path} className="flex items-center flex-1">
                <div className="flex items-center">
                  {isAccessible ? (
                    <Link
                      href={step.path}
                      className={`flex items-center space-x-3 rounded-2xl px-4 py-2 transition-smooth ${
                        stepStatus === 'current' ?'bg-primary text-primary-foreground shadow-neu-sm'
                          : stepStatus === 'completed' ?'bg-success text-success-foreground shadow-neu-sm' :'neu-button text-muted-foreground'
                      }`}
                    >
                    ...
                    <div className="flex items-center space-x-3 rounded-2xl px-4 py-2 neu-pressed text-muted-foreground cursor-not-allowed opacity-50">
                      <div className="flex h-10 w-10 items-center justify-center rounded-xl">
                        <Icon name={step.icon as any} size={20} variant="outline" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-sm font-medium">{step.label}</span>
                        <span className="caption text-xs opacity-75">Not available</span>
                      </div>
                    </div>
                  )}
                </div>
                    ...
                    <div className="h-1 neu-pressed rounded-full relative overflow-hidden">
                      <div
                        className={`h-full transition-smooth ${
                          stepStatus === 'completed' ? 'bg-success' : 'bg-transparent'
                        }`}
                      />
                    </div>
                  </div>
      ...
      <div className="md:hidden px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={`flex h-12 w-12 items-center justify-center rounded-2xl shadow-neu-sm ${
              currentStatus === 'approved' ? 'bg-success' : 'bg-primary'
            }`}>
              <Icon
            ...
            {steps.map((step) => {
              ...
              return isAccessible ? (
                <Link
                  key={step.path}
                  href={step.path}
                  className={`flex h-12 w-12 items-center justify-center rounded-2xl transition-smooth ${
                    pathname === step.path
